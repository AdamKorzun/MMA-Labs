...
class Task3:
    ...
    def separate_roots(polynom, root_range):
        ...
        if np.abs(mid) < 0.01:
            return
        roots = Task3.separate_roots(polynom, [root_range[0] + mid, root_range[1]]) # needs fix
        roots_2 = Task3.separate_roots(polynom, [root_range[0],root_range[1] - mid])
    ...
    def bisection(f, a, b, tol, counter = 0):
        ...
        if np.abs(f(m)) < tol:
            ...
            return Task3.bisection(f, m, b, tol, counter)
        elif np.sign(f(b)) == np.sign(f(m)):
            return Task3.bisection(f, a, m, tol, counter)
...
if __name__ == "__main__":
    ...
    for i in range(number_of_roots):
        sep_range = Task3.separate_roots(polynom, root_range.copy())
